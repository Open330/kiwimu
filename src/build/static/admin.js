    ...
    async function waitForTask(taskId, label) {
        ...
        while (true) {
            const response = await fetch('/api/tasks/' + encodeURIComponent(taskId), { headers: authHeaders });
    ...
    document.getElementById('btn-build').addEventListener('click', () => runAction('/api/build', '빌드'));

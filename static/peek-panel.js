...
(function () {
  ...
  const WIKI_LINK_RE = /^\/kiwimu\/wiki\/([^/?#]+)\.html(?:[?#].*)?$/;
  const STACK_LIMIT = 5;
  ...
  function build() {
    ...
    expandBtn.addEventListener('click', () => {
      if (currentSlug) {
        ...
        window.location.href = '/kiwimu/wiki/' + encodeURIComponent(currentSlug) + '.html';
      }
    });
  ...
  async function fetchAndRender(slug) {
    ...
    try {
      const resp = await fetch('/kiwimu/wiki/' + encodeURIComponent(slug) + '.html', {
        signal: ctrl.signal,
